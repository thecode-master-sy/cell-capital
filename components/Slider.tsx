...
const Slider: React.FC<React.HTMLAttributes<HTMLDivElement> & MotionProps> = ({
  className,
  children,
  ...props
}) => {
  ...
  useEffect(() => {
    ...
    setExtraSpace({
      width: sliderElement.current?.getBoundingClientRect().width ?? 0,
      height: sliderElement.current?.getBoundingClientRect().height ?? 0,
    });
  }, []);
  ...
  return (
    <div
      ...
      ref={sliderElement}
      className={cn("overflow-hidden select-none relative p-2", className)}
      {...props}
    >
      <motion.div
        ref={constraintsRef}
        drag="x"
        dragConstraints={{ left: -(constraint - extraSpace.width), right: 0 }}
